...
class SPINEModel(torch.nn.Module):
	...
	def __init__(self, params):
		...
		self.linear2=nn.Linear(self.hdim,self.h2dim)
        
        
		self.linear3 = nn.Linear(self.h2dim, self.inp_dim)
	...
	def forward(self, batch_x, batch_y):
		...
		#linear2_out=self.linear1_out
		h = linear2_out.clamp(min=0, max=1) # capped relu
		out = self.linear3(h)
